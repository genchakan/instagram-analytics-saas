...
export function Topbar({ onOpenMenu }: { onOpenMenu: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-bg/90 px-4 backdrop-blur-md sm:px-6">
        ...
        {account ? (
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden text-xs text-text-secondary sm:block">{t("topbar.lastSynced")}</p>
            </div>
          </div>
        ) : (
          <p className="truncate text-sm font-medium text-text-secondary">{t("topbar.noProfile")}</p>
        )}
      </div>
      ...
      <Avatar name={user?.fullName ?? t("topbar.you")} size="sm" />
    </header>
  );
}
